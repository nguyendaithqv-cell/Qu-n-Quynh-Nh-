...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 6. Synchronize Inventory Receipts list
    const unsubReceipts = onSnapshot(collection(db, 'inventoryReceipts'), async (snapshot) => {
      const list: InventoryReceipt[] = [];
      snapshot.forEach(docSnap => {
        list.push(docSnap.data() as InventoryReceipt);
      });
      list.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      setInventoryReceipts(list);
    }, (error) => {
      handleFirestoreError(error, OperationType.GET, 'inventoryReceipts');
    });
  ...
  // Real-time Inventory deduction and reversion when orders are submitted, modified, or cancelled.
  const syncInventoryForOrderChanges = async (
    originalOrdersList: Order[],
    updatedOrdersList: Order[],
    currentProductsList: Product[]
  ) => {
    const productsToUpdate = [...currentProductsList];
    const updatedProductIds = new Set<string>();

    const getProductQuantitiesInOrder = (order: Order) => {
    ...
    for (const updated of updatedOrdersList) {
      const original = originalOrdersList.find(o => o.id === updated.id);
      
      const originalQtys = original ? getProductQuantitiesInOrder(original) : {};
      const updatedQtys = getProductQuantitiesInOrder(updated);

      const allProdIds = new Set([
        ...Object.keys(originalQtys),
        ...Object.keys(updatedQtys)
      ]);

      for (const prodId of allProdIds) {
        const origQty = originalQtys[prodId] || 0;
        const updatedQty = updatedQtys[prodId] || 0;
        const delta = updatedQty - origQty; 

        if (delta !== 0) {
          const pIndex = productsToUpdate.findIndex(p => p.id === prodId);
          if (pIndex !== -1) {
            const product = productsToUpdate[pIndex];
            
            // 1. Direct stock tracking
            if (product.trackInventory) {
              const currentCount = product.inventoryCount || 0;
              const newCount = Math.max(0, currentCount - delta);
              
              // Automatically switch to out of stock on menu if quantity drops to 0
              const isAvailable = newCount > 0 ? product.isAvailable : false;

              productsToUpdate[pIndex] = {
                ...product,
            ...
            // 2. Recipe ingredient stock tracking (automated recipe deductions on sale)
            if (product.recipe && product.recipe.length > 0) {
              for (const recipeItem of product.recipe) {
                const ingredIndex = productsToUpdate.findIndex(p => p.id === recipeItem.ingredientId);
                if (ingredIndex !== -1) {
                  const ingredient = productsToUpdate[ingredIndex];
                  if (ingredient.trackInventory) {
                    const ingCurrentCount = ingredient.inventoryCount || 0;
                    const deduction = delta * recipeItem.quantity;
                    const ingNewCount = Math.max(0, ingCurrentCount - deduction);
                    const ingAvailable = ingNewCount > 0 ? ingredient.isAvailable : false;

                    productsToUpdate[ingredIndex] = {
                      ...ingredient,
                      inventoryCount: ingNewCount,
                      isAvailable: ingAvailable
                    };
                    updatedProductIds.add(recipeItem.ingredientId);
                  }
                }
              }
            }
    ...
    if (updatedProductIds.size > 0) {
      for (const prodId of updatedProductIds) {
        const p = productsToUpdate.find(x => x.id === prodId);
        if (p) {
          try {
            await setDoc(doc(db, 'products', p.id), cleanForFirestore(p));
          } catch (e) {
            console.error(`Failed to update inventory count for product ${p.name}:`, e);
          }
        }
      }
    }
  ...
  const handleAddOrder = async (newOrder: Order) => {
    ...
    try {
      // Deduct inventory immediately when order is submitted / sent card to kitchen
      await syncInventoryForOrderChanges([], [newOrder], products);
  ...
  const handleUpdateOrders = async (updatedOrders: Order[]) => {
    ...
    // Sync live inventory adjustments (cancelling adds stock back, updating menu quantities adjusts stock delta)
    await syncInventoryForOrderChanges(orders, modifiedOrders, products);
  ...
  const handleUpdateInventoryReceipts = async (updatedReceipts: InventoryReceipt[]) => {
    await syncWithFirestore('inventoryReceipts', updatedReceipts, inventoryReceipts);
  };
          ...
          {mobileMode === 'client' || mobileMode === 'history' || mobileMode === 'contact' ? (
            ...
            <div className="bg-white min-h-screen pb-16 flex flex-col">
              <AdminPanel
                ...
                onUpdateInventoryReceipts={handleUpdateInventoryReceipts}
