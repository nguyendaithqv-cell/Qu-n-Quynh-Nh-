...
export default function CashierPOS({
  ...
  const handleAddMenuItemToCart = (prod: Product) => {
    const exist = posCart.find(item => item.productId === prod.id);
    const currentCartQty = exist ? exist.quantity : 0;
    if (prod.trackInventory) {
      if (currentCartQty >= (prod.inventoryCount || 0)) {
        alert(`Món "${prod.name}" chỉ còn lại ${prod.inventoryCount || 0} sản phẩm trong kho! Không thể lấy thêm.`);
        return;
      }
    }
  ...
  const handleUpdateItemQuantityCart = (productId: string, val: number) => {
    if (val > 0) {
      ...
      if (prod && prod.trackInventory) {
        const exist = posCart.find(i => i.productId === productId);
        const currentQty = exist ? exist.quantity : 0;
        if (currentQty + val > (prod.inventoryCount || 0)) {
          alert(`Món "${prod.name}" chỉ còn lại ${prod.inventoryCount || 0} sản phẩm trong kho! Không thể lấy thêm.`);
          return;
        }
      }
  ...
  const filteredProductsMenu = useMemo(() => {
    return products.filter(p => {
      ...
      return matchSearch && matchCategory && p.isAvailable && !p.isIngredient;
    });
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                            <p className={`text-[10px] ${cm.textMuted} font-bold font-mono`}>{prod.price.toLocaleString('vi-VN')} đ</p>
                            {prod.trackInventory && (prod.inventoryCount || 0) <= (prod.lowStockAlert !== undefined ? prod.lowStockAlert : 10) && (
                              <span className="text-[8px] bg-amber-500/10 border border-amber-500/20 text-amber-600 rounded-md font-extrabold px-1.5 py-0.5 animate-pulse uppercase tracking-wide">
                                Còn {prod.inventoryCount || 0}
                              </span>
                            )}
                          </div>
                        </div>
