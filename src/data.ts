...
// Cookie helper funcs (Mirrored with localStorage for stability in preview environments)
export function setCustomerCookie(data: CustomerCookieData) {
  try {
    const jsonString = JSON.stringify(data);
    
    // Attempt standard cookie
    const date = new Date();
    ...
    localStorage.setItem('bep_viet_customer_backup', jsonString);
  } catch (e) {
...
export function getCustomerCookie(): CustomerCookieData | null {
  try {
    ...
    const backup = localStorage.getItem('bep_viet_customer_backup');
    if (backup) {
      return JSON.parse(backup) as CustomerCookieData;
    }
  } catch (e) {
